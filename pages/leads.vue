    ...
    <aside class="w-64 shrink-0 space-y-4">
      <div class="bg-slate-800/60 rounded-2xl border border-white/10 p-4 space-y-3">
        <h2 class="text-sm font-bold text-white flex items-center gap-2">
          <i class="pi pi-send text-indigo-400"></i> Sources
        </h2>
        ...
        <div class="space-y-2">
          ...
          >
            <div class="flex-1 min-w-0">
              <div class="text-xs font-medium text-white truncate">@{{ src.channel }}</div>
            ...
            <button
              @click="handleScrape(src.channel)"
            ...
            <button
              @click="removeSource(src.channel)"
          ...
          <div v-if="!sources.length" class="text-center py-6 text-slate-600 text-xs">
            Add a Telegram channel<br>to start collecting leads
          </div>
        </div>
        ...
        >
          <i class="pi pi-sync mr-1.5"></i> Scrape All
        </button>
      </div>
    ...
    <main class="flex-1 min-w-0 space-y-4">
      ...
      <div class="flex flex-wrap items-center gap-3">
        ...
        <input
          ...
          class="bg-slate-800 border border-white/10 rounded-lg px-3 py-1.5 text-sm text-white placeholder-slate-500 focus:outline-none focus:border-indigo-500 transition-colors w-56"
        />
      ...
      <div v-if="!filteredLeads.length && !leads.length" class="rounded-2xl bg-slate-800/40 border border-dashed border-white/10 py-20 text-center">
        ...
        <p class="text-slate-600 text-sm mt-1">Add a Telegram channel and click Scrape</p>
      </div>
        ...
        <table class="w-full text-sm">
          ...
          <tbody class="divide-y divide-white/5">
            <tr
              ...
              :key="lead.id"
              class="bg-slate-900/60 hover:bg-slate-800/60 transition-colors group"
            >
              <!-- Score -->
              <td class="px-4 py-3">
              ...
              <!-- Source -->
              <td class="px-4 py-3">
                <div class="text-xs text-slate-400">@{{ lead.sourceChannel }}</div>
                <div class="text-[10px] text-slate-600">{{ formatDate(lead.createdAt) }}</div>
              </td>
      ...
      >
        <div class="bg-slate-800 border border-white/15 rounded-2xl p-6 max-w-lg w-full space-y-4">
          <div class="flex items-center justify-between">
            <h3 class="font-bold text-white">Original Message</h3>
            <button @click="previewLead = null" class="text-slate-400 hover:text-white"><i class="pi pi-times"></i></button>
          </div>
          <p class="text-slate-300 text-sm whitespace-pre-wrap leading-relaxed">{{ previewLead.rawMessage }}</p>
          <div class="flex gap-3 text-xs text-slate-500">
            <span>@{{ previewLead.sourceChannel }}</span>
            <span>{{ formatDate(previewLead.createdAt) }}</span>
          </div>
        </div>
      </div>
    </main>
...
useHead({ title: 'Leads — NLP Data Analytics' })
...
async function handleScrape(channel: string) {
  setSourceStatus(channel, 'scraping')
  try {
    const res = await $fetch<{ leads: Lead[]; messagesScanned: number }>('/api/scrape', {
      method: 'POST',
      body: { channel }
    })
    addLeads(res.leads)
    updateSourceStats(channel, res.leads.length)
    setSourceStatus(channel, 'idle')
    showToast(`Scanned ${res.messagesScanned} messages, found ${res.leads.length} leads from @${channel}`)
  } catch (e: any) {
    const msg = e?.data?.message ?? e?.message ?? 'Scraping failed'
    setSourceStatus(channel, 'error', msg)
    showToast(msg, 'error')
  }
}
...
async function handleScrapeAll() {
  ...
  for (const src of idle) {
    await handleScrape(src.channel)
  }
}
